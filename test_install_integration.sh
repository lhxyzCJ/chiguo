...
grep -q "automations rm chiguo-check-old" "$CALLS_LOG" || fail "表格式 list 未 rm 旧作业 chiguo-check-old"
grep -q "automations rm chiguo-check\$" "$CALLS_LOG" && fail "表格式 list 误 rm 新作业 chiguo-check" || true
grep -q "hooks disable chiguo-old-hook\$" "$CALLS_LOG" || fail "表格式 hooks list 未禁用 chiguo-old-hook"
CP=$(grep -c "automations add --name chiguo-check" "$CALLS_LOG" || true)
