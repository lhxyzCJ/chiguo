...
| 🛡 Send-side reliability | agent failure → 5s jitter retry, then abort on still-failing (no composer fallback) + agent_health alert / manual recovery + content-level anti-repetition |
...
**Proactive sending**: a system crontab wakes `scripts/chiguo-tick.sh` every 15 minutes (or resident with `CHIGUO_DAEMON_LOOP=1`, see [AGENT_INTEGRATION.md](doc/AGENT_INTEGRATION.md)) → runs `chiguo_daemon.py --compact` as a **zero-LLM decision gate** (emotion / gating / triggers / topics all computed locally) → if the decision is not `send`, it exits; otherwise `scripts/agent-run.mjs --send-mode` (agent abstraction, the agent backend by default) turns the decision JSON into WeChat text via the LLM (dedicated session `chiguo-send`) → HTTP POST to the bridge `/send` for delivery → the send result is reported back to the daemon (`--record-send`). On agent generation failure there is **no composer fallback**: the first failure sleeps 5s and retries the full chain once (jitter buffer; a retry success is not a fault); if it still fails the send is **aborted** and counted toward agent_health — after consecutive failures cross the threshold a "backend abnormal" alert is sent over WeChat and probing pauses; after the fix it resumes via restart (loop) or the next cron probe success (a "recovered" transition message).

**Passive replying**: a WeChat message enters the bridge → an **OWNER_ID gate** (non-owners only get a plain chat reply — no accounting, no command/recall paths) → `chiguo_daemon.py --user-msg --recv-id <uuid>` records it **deterministically** (the bridge generates a per-message uuid locally; real-time emotion response, recv_dedup dedupes exactly by id) → `command-detect.mjs` rules check first: **special commands** (anniversaries / holidays / break on-off) are executed and answered directly, **no LLM**; **schedule write-commands** (cancel / move / add / exam week / reminder / remove) go through `agent-run.mjs --schedule-extract` extraction → `--schedule-verify` verification (dual agents, separate sessions; if info is missing they return a question into the clarify loop, records valid 6h) → daemon `--schedule-change` atomic write (confirmation text carries weekday + date); ordinary messages first fetch a lightweight `--attention` injection (today's important days / active range facts / this week's schedule), then run `agent-run.mjs --analysis-mode`, producing "mood analysis JSON + reply text" in one call — if the analysis carries a recall signal (registered facts / past dates), the bridge queries the facts and answers via a second agent pass → the analysis is merged back into the daemon via `--analysis` (same-id upgrade only fine-tunes, never double-counts) → the reply is sent back to WeChat. The reply side runs as a resident serial process (TurnQueue, session `chiguo-main`), zero session sharing with proactive sending.

**Shared & alerting**: daemon state is written atomically to `chiguo_state.json` (tmp→os.replace + checksum), decisions appended to `chiguo_decisions.jsonl`; memory (`[memory].backend`, mem0 the only backend) and the NetEase Music bridge feed topic inputs; `chiguo_monitor.py` patrols independently. Both pipelines record agent-call outcomes into the `agent_health.py` liveness state machine — when consecutive failures cross the threshold, it alerts via the WeChat bridge automatically, and notifies on recovery (zero extra LLM calls).
...
flowchart LR
    ...
    subgraph 被动回复链
        ...
        BR --> UR[daemon --user-msg --recv-id<br/>记账 recv_dedup]
...
**Missing**: a hard deployment check — the memory check in `chiguo_envcheck.py` reports warn/critical: mem0ai not installed → critical (blocks deployment; `uv sync` installs it); no LLM key / qdrant not initialized → warn (memory topic sources shrink, non-fatal). At runtime an unavailable store is reprobed every 60s and self-heals after recovery. Differences in detail: [❓ FAQ](#-faq).
...
**context_token lifetime (known behavior, not a fault)**: proactive sends depend on a context_token issued by the WeChat server (stored in `~/.chiguo/auth/wechat/context_tokens.json`). WeChat does not publish a TTL; measured in practice, it **expires about 35 hours after the last received user message** and is auto-refreshed on every incoming user message, so normal back-and-forth never hits expiry. The only symptom is a failed proactive send: `[send error] prepare failed` (the reply path is unaffected) — **send one message from WeChat to the bot to recover**, no re-scan or service restart needed. Diagnostics: `bash scripts/wechat-bridge.sh status` shows token freshness (and `deploy.sh` checks it at deploy time).
...
memory/                  # memory backend abstraction (mem0 only backend; base/mem0_backend/factory/cli)
