...
**A role-play AI companion who proactively messages her owner**
...
| Memory system (LanceDB + ollama embedding) | Optional | Memory topic sources reduced (JSON fallback); `envcheck` warns |
| NetEase Music bridge | Optional | Fully inactive (one fewer topic source) |
| Class schedule xlsx | Optional | Treated as free time (availability=1.0) |
...
test_*.py / .mjs / .sh   # tests (standalone runners)
