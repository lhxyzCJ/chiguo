...
```bash
bash scripts/ci-test.sh   # full suite (43 py + 10 script) — 本地与 GitHub Actions ci.yml 同一入口；任一失败退出非零
```
...
1. Run affected test files; full chain (43 py + 10 script) if touching math/state/daemon.
