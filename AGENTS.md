...
- Everything tunable in `chiguo_proactive.toml` (470 lines, 22 sections); hot-reloads via mtime check in `--loop` mode only (cron spawns fresh processes).
- Output: `chiguo_decisions.jsonl` (append-only). State: `chiguo_state.json` (atomic `.tmp` → `os.replace`, `.bak` backup, SHA256 checksum, monotonic `tick_seq`). Privacy runtime files (state/decisions/messages/login state) and monitoring/health JSONL (audit/health) are `.gitignore`d (local only, history rewritten).
