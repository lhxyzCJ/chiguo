...
import { ContextStore, InboundDebouncer, MessagePoller, MessageSender, TypingService } from '../messaging/index.js'
import type { InboundDebounceOptions } from '../messaging/index.js'
...
export interface WeChatBotOptions {
  ...
  botAgent?: string
  /**
   * Inbound message debounce: batches rapid text messages from the same
   * sender into a single dispatch (trailing window). Media/quoted messages
   * flush the buffer immediately and are never merged. Default: disabled.
   */
  inboundDebounce?: InboundDebounceOptions
}
...
export class WeChatBot extends TypedEmitter<BotEventMap> {
  ...
  private inboundDebouncer: InboundDebouncer | null = null
  ...
  constructor(options: WeChatBotOptions = {}) {
    ...
    // Optional inbound debounce (trailing window merge per sender)
    const debounceCfg = options.inboundDebounce
    if (debounceCfg && debounceCfg.windowMs > 0) {
      this.inboundDebouncer = new InboundDebouncer({
        ...debounceCfg,
        onFlush: (merged) => {
          this.emit('message', merged)
          this.dispatchToHandlers(merged)
        },
      })
    }
  }
  ...
  stop(): void {
    this.poller.stop()
    this.inboundDebouncer?.flushAll()
  ...
  private setupPollerEvents(): void {
    ...
    this.poller.on('messages', async (messages) => {
      for (const wire of messages) {
        ...
        // Inbound debounce: buffer rapid same-sender texts, flush as one message
        if (this.inboundDebouncer && this.inboundDebouncer.enqueue(incoming)) continue
