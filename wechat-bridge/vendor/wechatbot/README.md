...
const bot = new WeChatBot({
  ...
  // 入站消息抖动合并（可选）：窗口内同一发送者的连续文本消息合并为一次分发
  // 0/缺省 = 禁用；媒体/引用消息立即分发，不参与合并
  inboundDebounce: {
    windowMs: 4000,
    joinSeparator: '\n',
  },
...
### Inbound debounce (消息抖动合并)

微信用户常把一句话拆成多条连发，逐条触发 agent 会浪费模型调用且上下文割裂。
`inboundDebounce` 将窗口内同一发送者的连续纯文本消息合并为一条后分发：

```typescript
const bot = new WeChatBot({
  inboundDebounce: {
    windowMs: 4000,      // 合并窗口（ms）。0 = 禁用（默认）
    joinSeparator: '\n', // 合并文本分隔符（默认换行）
  },
})

bot.onMessage(async (msg) => {
  // 4 秒内连发的 "在吗" + "帮我查下天气" 会合并为 msg.text = "在吗\n帮我查下天气"
  await bot.reply(msg, `Echo: ${msg.text}`)
})
```
...
- 仅纯文本消息参与合并；媒体（图片/文件/语音/视频）与引用消息立即分发，并同时冲刷该发送者的待合并文本
- 合并窗口为 trailing 语义：每条消息重置定时器，窗口过期后统一分发
- 合并消息保留最后一条消息的 `timestamp`/`raw`/`_contextToken`，媒体列表为空
