#!/usr/bin/env bash
# 全量测试链（43 py + 10 script）——本地与 CI 同一入口；任一失败即退出非零
...
uv run python tests/test_emotion_noise.py && uv run python tests/test_emotion_baseline.py && uv run python tests/test_loop_send.py && \
