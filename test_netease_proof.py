...
def _make_engine(tmp, factor=None):
    ...
    cfg_path.write_text(_toml_variant(factor))
...
if __name__ == "__main__":
    ...
    test_cache_hit_and_copy()
    test_cache_expiry()
    test_api_failure_and_corrupt_cache()
    test_invalid_playtime_filtered_and_error_codes()
    test_non_dict_resp_and_data_return_none()
    test_cache_path_injection()
    test_not_logged_in()
    test_future_fetched_at_not_hit()
    ...
    test_check_play_proof_outside_window_no_fetch()
    test_check_play_proof_stale_play_no_proof()
    test_check_play_proof_fetch_none_or_exception()
    ...
    test_api_get_retries_http_5xx()
    test_api_get_retry_policy_zero()
    test_daily_songs_schema_filter()
    test_daily_songs_non_dict_resp()
    test_pick_netease_only_injection_rules()
    print("test_netease_proof.py: ALL PASS")
