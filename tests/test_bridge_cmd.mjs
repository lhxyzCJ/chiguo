...
t('#130: 正例保持——"2026年12月25日要考试" 仍 reminder_added、label 含考试', () => {
  const r = detectSpecialCommand('2026年12月25日要考试')
  assert.strictEqual(r.action, 'reminder_added')
  assert.ok(r.payload.label.includes('考试'), JSON.stringify(r.payload))
})
...
t('executeSpecialCommand: 非法 payload（未知 kind）→ ok:false + 处理失败（不抛异常）', async () => {
  const spec = { action: 'x', payload: { kind: 'nope' }, hint: 'hint' }
  const r = await executeSpecialCommand(spawn, spec, process.execPath, FAKE_DAEMON)
  assert.strictEqual(r.ok, false)
  assert.ok(r.reply.startsWith('处理失败'), r.reply)
})
t('executeSpecialCommand: 非法 payload（日期格式错/空名称/控制字符）→ ok:false', async () => {
  for (const payload of [
    { kind: 'anniversary_add', date: '5-11', name: 'x' },
    { kind: 'anniversary_add', date: '13-01', name: 'x' },
    { kind: 'anniversary_add', date: '05-11', name: '' },
    { kind: 'anniversary_add', date: '05-11', name: 'a\nb' },
    { kind: 'reminder', date: '2026-13-01', label: 'x' },
    { kind: 'schedule_change', item: { when: { date: '2026-08-20' } } },
    null,
    'not-object',
  ]) {
    const r = await executeSpecialCommand(spawn, { action: 'x', payload, hint: 'hint' }, process.execPath, FAKE_DAEMON)
    assert.strictEqual(r.ok, false, JSON.stringify(payload))
    assert.ok(r.reply.startsWith('处理失败'), r.reply)
  }
})
t('executeSpecialCommand: 缺 action → ok:false', async () => {
  const r = await executeSpecialCommand(spawn, { payload: { kind: 'break_on' }, hint: 'hint' }, process.execPath, FAKE_DAEMON)
  assert.strictEqual(r.ok, false)
  assert.ok(r.reply.startsWith('处理失败'), r.reply)
})
...
t('executeSpecialCommand: --schedule-change 成功 shape(A4)→ ok:true(确认文案含星期+日期)', async () => {
  const spec = { action: 'schedule_change', payload: { kind: 'schedule_change', item: { kind: 'reminder', when: { date: '2026-08-20' }, label: '交材料' } }, hint: 'x' }
  const r = await executeSpecialCommand(spawn, spec, process.execPath, FAKE_DAEMON)
  assert.strictEqual(r.ok, true)
})
...
t('executeSpecialCommand: schedule_change item 缺 kind → DTO 白名单前置拒绝 ok:false（畸形到不了 daemon）', async () => {
  const spec = { action: 'schedule_change', payload: { kind: 'schedule_change', item: { when: { date: '2026-08-20' } } }, hint: 'x' }
  const r = await executeSpecialCommand(spawn, spec, process.execPath, FAKE_DAEMON)
  assert.strictEqual(r.ok, false)
  assert.ok(r.reply.startsWith('处理失败'), r.reply)
})
t('executeSpecialCommand: --schedule-change ApiRejection shape(A4 reason+question+missing)→ ok:false', async () => {
  const spec = { action: 'schedule_change', payload: { kind: 'schedule_change', item: { kind: 'reminder', when: { date: '2026-08-01' }, label: '过去' } }, hint: 'x' }
  const r = await executeSpecialCommand(spawn, spec, process.execPath, FAKE_DAEMON)
  assert.strictEqual(r.ok, false)
})
t('executeSpecialCommand: --schedule-change remove 拒绝(not_found)→ ok:false 且失败 JSON 不丢 stdout', async () => {
  const spec = { action: 'schedule_change', payload: { kind: 'schedule_change', item: { kind: 'remove', match: { date: '2026-08-20' } } }, hint: 'x' }
  const r = await executeSpecialCommand(spawn, spec, process.execPath, FAKE_DAEMON)
  assert.strictEqual(r.ok, false)
  assert.ok(r.reply.startsWith('处理失败'), r.reply)
})
