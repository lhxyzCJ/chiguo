...
def _guard_state_files_isolation():
    ...
    yield
    # 校验 decisions/messages/audit 无新增行
    for name, path in [("chiguo_decisions.jsonl", decisions),
        ...
        base = baselines[name.split(".")[0] if "audit" not in name else "audit"]
        # decisions/messages 允许不变，audit 同理（基线 vs 当前）
        # 统一用 decisions 键映射
    ...
    # state.json 大小不应无故增长过多（允许 ±1KB 抖动，因 save 可能格式化差异）
    if state_p.exists():
        now_size = state_p.stat().st_size
        # 严格：大小不应显著增长（>2KB 视为污染）
