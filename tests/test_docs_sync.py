...
#    ci-test.sh 实际测试集合 ↔ 文档计数（59 py + 14 script）
...
check("ci-test.sh 运行 9 个 node 测试", len(mjs_tests) == 9, f"实际 {len(mjs_tests)}")
...
for f in ["AGENTS.md", "CLAUDE.md", "README.md", "README_EN.md",
    ...
    check(f"{f} 计数为 59 py + 14 script", any((int(a), int(b)) == (59, 14) for a, b in nums), f"实际 {nums}")
