...
#    ci-test.sh 实际测试集合 ↔ 文档计数（37 py + 10 script）
...
check("ci-test.sh 运行 37 个 py 测试", len(py_tests) == 37, f"实际 {len(py_tests)}")
...
for f in ["AGENTS.md", "CLAUDE.md", "README.md", "README_EN.md"]:
    ...
    check(f"{f} 计数为 37 py + 10 script", any((int(a), int(b)) == (37, 10) for a, b in nums), f"实际 {nums}")
